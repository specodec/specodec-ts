export class GronWriter {
  ...
  private segments: string[] = ["json"];
  private nestingStack: { depth: number; arrayIndex?: number }[] = [];

  private buildPath(): string {
    let r = this.segments[0];
    for (let i = 1; i < this.segments.length; i++) {
      if (this.segments[i].startsWith("[")) r += this.segments[i];
      else r += "." + this.segments[i];
    }
    return r;
  }
  ...
  private escape(s: string): string {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      switch (c) {
        ...
        case 0x09: r += "\\t"; break;
        default:
          if (c < 0x20) r += "\\u" + c.toString(16).padStart(4, "0");
          else r += s.charAt(i);
      }
    }
    return r;
  ...
  writeFloat32(value: number): void {
    if (!isFinite(value)) throw new Error("float32: NaN/Infinity not valid");
