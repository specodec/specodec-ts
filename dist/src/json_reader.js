export class JsonReader {
    ...
    peek() {
        ...
        if (this._pos >= this.src.length)
            throw new Error("json: unexpected end of input");
    ...
    expect(ch) {
        const got = this.read();
        if (got !== ch)
            throw new Error(`json: expected '${ch}', got '${got}' at position ${this._pos - 1}`);
    }
    parseString() {
        this.expect('"');
        let r = "";
        while (this._pos < this.src.length) {
            ...
            if (c === 0x5C) {
                this._pos++;
                if (this._pos >= this.src.length)
                    throw new Error("json: unexpected end of string escape");
                ...
                switch (esc) {
                    ...
                    case 0x75: {
                        this._pos++;
                        if (this._pos + 4 > this.src.length)
                            throw new Error("json: incomplete unicode escape");
                        const hex = this.src.slice(this._pos, this._pos + 4);
                        let cp = parseInt(hex, 16);
                        if (isNaN(cp))
                            throw new Error(`json: invalid unicode escape \\u${hex}`);
                        this._pos += 4;
                        if (cp >= 0xD800 && cp <= 0xDBFF) {
                            if (this._pos + 6 <= this.src.length &&
                                ...
                                if (isNaN(low))
                                    throw new Error(`json: invalid low surrogate \\u${hex2}`);
                                this._pos += 4;
    ...
    parseNumberRaw() {
        ...
        if (this.src.charCodeAt(this._pos) === 0x2D)
            this._pos++;
        if (this._pos >= this.src.length)
            throw new Error("json: unexpected end of number");
        if (this.src.charCodeAt(this._pos) === 0x30) {
            this._pos++;
        }
        else if (this.src.charCodeAt(this._pos) >= 0x31 && this.src.charCodeAt(this._pos) <= 0x39) {
            this._pos++;
            while (this._pos < this.src.length && this.src.charCodeAt(this._pos) >= 0x30 && this.src.charCodeAt(this._pos) <= 0x39)
                this._pos++;
        }
        else {
        ...
        if (this._pos < this.src.length && (this.src.charCodeAt(this._pos) === 0x65 || this.src.charCodeAt(this._pos) === 0x45)) {
            this._pos++;
            if (this._pos < this.src.length && (this.src.charCodeAt(this._pos) === 0x2B || this.src.charCodeAt(this._pos) === 0x2D))
                this._pos++;
            if (this._pos >= this.src.length || this.src.charCodeAt(this._pos) < 0x30 || this.src.charCodeAt(this._pos) > 0x39) {
    ...
    readBool() {
        ...
        if (ch === "t") {
            for (const c of "true") {
                if (this.read() !== c)
                    throw new Error("json: expected 'true'");
            }
            return true;
        }
        if (ch === "f") {
            for (const c of "false") {
                if (this.read() !== c)
                    throw new Error("json: expected 'false'");
            }
            return false;
        }
    ...
    readInt32() {
        ...
        if (!Number.isInteger(v))
            throw new Error(`json: int32 must be integer, got ${raw}`);
        if (v < -2147483648 || v > 2147483647)
            throw new Error(`json: int32 overflow: ${raw}`);
        return v | 0;
    }
    readInt64() {
        const ch = this.peek();
    ...
    readUint32() {
        ...
        const v = Number(raw);
        if (!Number.isInteger(v))
            throw new Error(`json: uint32 must be integer, got ${raw}`);
        if (v < 0 || v > 4294967295)
            throw new Error(`json: uint32 overflow: ${raw}`);
        return v >>> 0;
    }
    readUint64() {
        const ch = this.peek();
    ...
    readNull() {
        for (const c of "null") {
            if (this.read() !== c)
                throw new Error("json: expected 'null'");
        }
        return null;
    }
    readBytes() {
        const s = this.parseString();
        if (s.length % 4 !== 0)
            throw new Error("json: invalid base64 length");
        ...
        for (let i = 0; i < s.length; i += 4) {
            ...
            if (a < 0 || b < 0)
                throw new Error("json: invalid base64");
            buf[bi++] = (a << 2) | (b >> 4);
            if (s[i + 2] !== "=" && c >= 0) {
                buf[bi++] = ((b & 0xF) << 4) | (c >> 2);
                if (s[i + 3] !== "=" && d >= 0) {
    ...
    skip() {
        ...
        switch (ch) {
            ...
            case "t": {
                for (const c of "true") {
                    if (this.read() !== c)
                        throw new Error("json: skip expected true");
                }
                return;
            }
            case "f": {
                for (const c of "false") {
                    if (this.read() !== c)
                        throw new Error("json: skip expected false");
                }
                return;
            }
            case "n": {
                for (const c of "null") {
                    if (this.read() !== c)
                        throw new Error("json: skip expected null");
                }
                return;
            }
            default:
